...
def detalleEmpleado(idEmpleado=None):
    if 'conectado' in session:
        ...
        if idEmpleado is None:
            ...
            print(prediccion_asfixia)

            prediccion_asfixia_prob = voting_model.predict_proba(datos_paciente)
            print(prediccion_asfixia_prob)

            if prediccion_asfixia[0] > 0:
                detalle_empleado['asfixia'] = 'Probabilidad'
                detalle_empleado['asfixia_prob'] = round(prediccion_asfixia_prob[0][1] * 100, 2)
            else:
                detalle_empleado['asfixia'] = 'Probabilidad'
                detalle_empleado['asfixia_prob'] = round(prediccion_asfixia_prob[0][1] * 100, 2)
